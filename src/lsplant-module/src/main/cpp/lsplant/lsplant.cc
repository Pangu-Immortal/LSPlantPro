...
// VA 修复（A16 分身无法启动根因，2026-06-27）：
//   Android 16/API36 移除 DexFile(ByteBuffer) 构造 + libart 无 DexFile::OpenMemory 符号（改 DexFileLoader::Open，
//   签名返回 vector<unique_ptr<DexFile>> 不兼容），LSPlant 无法加载 hooker dex → hook backup null
//   → VA Java method hook 全失效 → 分身崩溃/ANR（即刻/高德 native crash，抖音/BBKClock ANR）。
//   用 InMemoryDexClassLoader（API26+ @SystemApi，JNI FindClass 可达，不受 hidden API 限制）加载 hooker dex，
//   纯 Java 层不依赖 art native 符号，绕过 OpenMemory 缺失。
jmethodID in_memory_dex_cl_init = nullptr;    // dalvik.system.InMemoryDexClassLoader.<init>(ByteBuffer, ClassLoader)
jmethodID class_loader_load_class = nullptr;  // java.lang.ClassLoader.loadClass(String)Class
...
bool InitJNI(JNIEnv *env) {
    ...
    // VA 修复（A16 根因）：InMemoryDexClassLoader 路径初始化。
    //   API26+ 引入 dalvik.system.InMemoryDexClassLoader（@SystemApi，JNI 可达），
    //   A16 DexFile(ByteBuffer) 移除后用它加载 hooker dex。ClassLoader.loadClass(String) 单参用于加载 hooker 类。
    //   不进 kInternalMethods（非必需，旧设备/OpenMemory 可用时不用），失败不阻断 InitJNI。
    if (sdk_int >= kSdkOreo) {
        auto in_mem_cl = JNI_FindClass(env, "dalvik/system/InMemoryDexClassLoader");
        if (in_mem_cl) {
            in_memory_dex_cl_init = JNI_GetMethodID(env, in_mem_cl, "<init>",
                                                    "(Ljava/nio/ByteBuffer;Ljava/lang/ClassLoader;)V");
        }
        auto cl_class = JNI_FindClass(env, "java/lang/ClassLoader");
        if (cl_class) {
            class_loader_load_class = JNI_GetMethodID(env, cl_class, "loadClass",
                                                     "(Ljava/lang/String;)Ljava/lang/Class;");
        }
        if (sdk_int >= kSdkBaklava && !in_memory_dex_cl_init) {
            // A16+ DexFile(ByteBuffer) 已移除，InMemoryDexClassLoader 是唯一纯 Java 兜底，不可用则告警。
            LOGW("InMemoryDexClassLoader.<init> not found, A16+ hooker dex load will fail");
        }
    }
    ...
    if (sdk_int >= kSdkOreo && !dex_file_init_with_cl && !dex_file_init) {
        // Android 16/API36 已移除 ByteBuffer 形态 DexFile 构造函数。
        // 这里不能让 InitJNI 失败，后续 CreateHookerClass 会降级走 native OpenMemory。
        LOGW("DexFile ByteBuffer constructor missing, fallback to native OpenMemory");
    }
    if (load_class =
...
std::tuple<jclass, jfieldID, jmethodID, jmethodID> BuildDex(JNIEnv *env, jobject class_loader,
    ...
    // VA 修复（A16 根因）：InMemoryDexClassLoader 路径产物，A16 用它替代 DexFile/OpenMemory 加载 hooker dex。
    ScopedLocalRef<jobject> hooker_cl{nullptr};
    ...
    if (auto dex_file_class = JNI_FindClass(env, "dalvik/system/DexFile"); dex_file_init_with_cl) {
        ...
        //   （API26+ @SystemApi，纯 Java 不依赖 art OpenMemory 符号，A16 标准 dex 加载方案）。
        if (in_memory_dex_cl_init && class_loader_load_class) {
            auto imcl_class = JNI_FindClass(env, "dalvik/system/InMemoryDexClassLoader");
            if (imcl_class) {
                // NewDirectByteBuffer 包装 hooker dex 字节流，InMemoryDexClassLoader 直接加载内存 dex。
            ...
            if (env->ExceptionCheck()) {
                ...
                env->ExceptionClear();
                hooker_cl = ScopedLocalRef<jobject>(env, nullptr);
            }
        }
        if (!hooker_cl) {
    ...
    if (hooker_cl) {
        // VA 修复（A16 根因）：InMemoryDexClassLoader 路径，ClassLoader.loadClass(String) 单参加载 hooker 类。
        target_class =
            JNI_Cast<jclass>(
                JNI_CallObjectMethod(env, hooker_cl, class_loader_load_class,
